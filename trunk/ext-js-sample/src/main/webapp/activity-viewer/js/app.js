...
Ext.application({
    requires: ['LiveGridApp.store.QuoteStore', 'LiveGridApp.view.QuoteGrid']
    , appFolder: '/activity-viewer/js'
    , name   : 'LiveGridApp'

    , launch : function() {

        var quoteStore = Ext.create('LiveGridApp.store.QuoteStore', {});
        var quoteGrid = Ext.create('LiveGridApp.view.QuoteGrid', {
            store: quoteStore
        });

        Ext.create('Ext.container.Viewport', {
            requires: ['LiveGridApp.store.QuoteStore']
            , width: 500
            , height: 300
            , layout: 'border'
            , items: [
