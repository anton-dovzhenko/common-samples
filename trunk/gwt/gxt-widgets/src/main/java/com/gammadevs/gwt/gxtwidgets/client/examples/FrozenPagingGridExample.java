...
import com.gammadevs.gwt.gxtwidgets.client.widgets.FrozenPagingGrid;
...
public class FrozenPagingGridExample implements IsWidget {

    interface FrozenGridExampleUiBinder extends UiBinder<HorizontalLayoutContainer, FrozenPagingGridExample> {}
    ...
    FrozenPagingGrid<FrozenGridModel> grid;

    public FrozenPagingGridExample() {
        ...
        grid = new FrozenPagingGrid<>(props.key(), lockedColumns, columns, new PagingLoader<>(new FrozenPagingProxy()));

        rootElement = ourUiBinder.createAndBindUi(this);

    }

    private class FrozenPagingProxy extends RpcProxy<PagingLoadConfig, PagingLoadResult<FrozenGridModel>> {

        @Override
        public void load(PagingLoadConfig loadConfig, AsyncCallback<PagingLoadResult<FrozenGridModel>> callback) {
            int limit = loadConfig.getLimit();
            int offset = loadConfig.getOffset();
            int totalLength = 1006;
            List<FrozenGridModel> items = new ArrayList<>();
            for (int i = offset; i <= Math.min(limit, totalLength - offset) + offset - 1; i++) {
                items.add(new FrozenGridModel(10 * i
                        , new Date(i * 533)
                        , i * Math.random()
                        , i, i, i, i, i, i, i, i, i, i, i, i));
            }
            callback.onSuccess(new PagingLoadResultBean<>(items, totalLength, offset));
        }
